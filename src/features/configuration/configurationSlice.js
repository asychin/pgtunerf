...
export const selectSharedBuffers = createSelector(
  ...
  (totalMemoryKb, dbType, osType, dbVersion) => {
    ...
    // per PostgreSQL wiki recommendation for large memory systems
...
export const selectHugePages = createSelector(
  [selectSharedBuffers],
  // enable huge_pages when shared_buffers >= 8GB (8388608 kB)
  (sharedBuffersKb) => (sharedBuffersKb >= 8388608 ? 'try' : 'off')
)
...
export const selectEffectiveIoConcurrency = createSelector(
  [selectOSType, selectHDType, selectDBVersion],
  (osType, hdType, dbVersion) => {
    // Windows supports effective_io_concurrency starting from PostgreSQL 13
    if (osType === OS_WINDOWS && dbVersion < 13) {
      return null
    }
    // macOS does not support effective_io_concurrency
    if (osType !== OS_LINUX && osType !== OS_WINDOWS) {
      return null
    }
    return {
      [HARD_DRIVE_HDD]: 2,
...
export const selectWorkMem = createSelector(
  ...
  (
    ...
    // Use max_parallel_workers_per_gather for work_mem calculation
    // since it reflects actual per-query parallelism, not total worker count
    const parallelForWorkMem = (() => {
      if (parallelSettingsValue.length) {
        const workersPerGather = parallelSettingsValue.find(
          (param) => param['key'] === 'max_parallel_workers_per_gather'
        )
        if (workersPerGather && workersPerGather['value'] && workersPerGather['value'] > 0) {
          return workersPerGather['value']
        }
      }
      if (
